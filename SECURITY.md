...
GEO SEO Hub `0.1.x` is experimental. Security fixes are applied to the latest commit on `main`; users should reproduce a report against that revision when practical.
...
Reports may cover the Python runtime, Skill routing, Artifact Bus publication, input and archive handling, package installation, or accidental disclosure through generated artifacts. Third-party platform behavior and unsupported planned integrations remain outside the executable `0.1.x` scope.
