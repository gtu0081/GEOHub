...
3. Run `python -m yao_geo content --input <brief.json> --output <runs-root>`.
...
Run offline. Treat only supplied evidence as factual. Keep unsupported material as guidance, `unverified`, `source_gap`, or `blocked-by-evidence`. Render HTML locally with escaped text and no external assets. Optional DOCX/PDF failures degrade explicitly while core artifacts remain usable.
