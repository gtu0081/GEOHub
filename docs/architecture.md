...
6. Domain intelligence modules implement discovery, explicit-source audit, bounded site discovery and audit, content, measurement, optimization, and knowledge governance behind public compatibility façades.
...
`geo-discover`, `geo-content`, `geo-measure`, `geo-strategy`, and `geo-knowledge` are offline. `geo-diagnose` supports replay snapshots and a tightly bounded explicit-source network mode. `geo-site-diagnose` approves one public host, discovers at most 500 candidate URLs, selects at most ten representative pages, and can render JavaScript shells through an optional restricted browser. `geo-publish` has a null entrypoint and cannot execute. `strategy-observation-loop` moves through an approval node and two schema-validated external evidence nodes; it never performs publication or platform collection.

Dedicated SEO providers must preserve the same evidence, permission, output, and degradation contracts before they become runnable. Architecture decisions are recorded under `docs/decisions/`.
